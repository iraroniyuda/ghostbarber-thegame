...
public class TrackManager : MonoBehaviour
{
    ...
    public IEnumerator Begin()
    {
        if (!m_Rerun)
        {
            ...
            // Validate if there are any characters in the list and that usedCharacter is a valid index
            if (PlayerData.instance.characters.Count == 0)
            {
                Debug.LogError("No characters available in PlayerData!");
                yield break;  // Exit if no characters are found
            }

            if (PlayerData.instance.usedCharacter < 0 || PlayerData.instance.usedCharacter >= PlayerData.instance.characters.Count)
            {
                Debug.LogWarning("Invalid usedCharacter index. Resetting to default character.");
                PlayerData.instance.usedCharacter = 0;  // Reset to the first character if the index is invalid
            }
            ...
            {
                Debug.LogWarning($"Unable to load character {PlayerData.instance.characters[PlayerData.instance.usedCharacter]}.");
                yield break;
            }
